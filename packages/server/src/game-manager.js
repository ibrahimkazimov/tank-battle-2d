import { GAME_CONFIG, PLAYER_CONFIG, EVENTS, Physics, COLORS, BULLET_CONFIG, WORLD_BOUNDS } from '@tank-battle/shared';
...
export class GameManager {
  ...
  findSafeSpawnPosition() {
    ...
    for (let i = 0; i < maxAttempts; i++) {
        const x = safeBounds.left + Math.random() * (safeBounds.right - safeBounds.left);
        const y = safeBounds.top + Math.random() * (safeBounds.bottom - safeBounds.top);

        let collides = false;
        // Check walls
        for (const wall of this.walls) {
            if (x + padding > wall.x && x - padding < wall.x + wall.width &&
    ...
    setInterval(() => {
        ...
        for (let i = this.bullets.length - 1; i >= 0; i--) {
            ...
            if (bullet.destroying) {
                if (Date.now() - bullet.destructionStartTime > BULLET_CONFIG.DESTRUCTION_TIME) {
            ...
            // Check world bounds
             if (bullet.x < WORLD_BOUNDS.LEFT * 2 || bullet.x > WORLD_BOUNDS.RIGHT * 2 || 
                 bullet.y < WORLD_BOUNDS.TOP * 2 || bullet.y > WORLD_BOUNDS.BOTTOM * 2) {
                this.bullets.splice(i, 1);
                continue;
            }

            // Check collisions
            const collision = Physics.checkBulletCollisions(bullet, this.players, this.walls);
            ...
            if (collision) {
                ...
                if (collision.type === 'player') {
                    ...
                    player.knockbackX = knockbackDirX * BULLET_CONFIG.POWER * BULLET_CONFIG.SPEED;
                    player.knockbackY = knockbackDirY * BULLET_CONFIG.POWER * BULLET_CONFIG.SPEED;
